...
# YeaHii add method
def add_dict(thedict,key_a,key_b,val):
    if key_a in thedict:
    ...
    if len(sys.argv) < 2:
        ...
        try:
            ...
            if re.match(r'^https?:/{2}\w.+$', target_url):
                ...
                cms_name = ''
                dicts = {}
                filePath = "../Web_Information/Restore.txt"
                cms_name_flag = 0
                ...
                Cms_name = "Cms_name"
                # Yeahii add
                add_dict(dicts,target_url,Banner,banner_all)
                add_dict(dicts,target_url,Cms_name,cms_name)
                with open(filePath, 'a+', encoding='utf8') as br:
                    json.dump(dicts,br)
                    br.write("\n")
